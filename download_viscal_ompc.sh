...
export base_directory=$GWS_PATH/s3_slstr_raw_data/$model/flight/vsc_ax

export temp_viscal_list=$base_directory/temp_viscal_filelist.txt

/usr/bin/python2.7 $GWS_PATH/software/slstr_calibration_ftp_retriever/data_retriever.py -c $GWS_PATH/software/slstr_calibration_ftp_retriever/config/slstr_cpa_viscal_ompc_$model.cfg -v -L -p /$model/SL_1_VSC_AX/$process_date #> $temp_viscal_list
...
export temp_config=$GWS_PATH/software/slstr_calibration_ftp_retriever/config/slstr_cpa_mpc_viscal_temp_$model.cfg
...
while read -r line; do 
   ...
   echo ftp_host: ftp.adwaiseo-services.com >>$temp_config
   echo ftp_user: ftp_ompc_data >>$temp_config
   echo ftp_pw: Caing7ee  >>$temp_config
   ...
   echo ftp_server_path: /$model/SL_1_VSC_AX/$process_date/$id >>$temp_config
